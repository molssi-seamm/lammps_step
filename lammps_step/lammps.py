...
class LAMMPS(seamm.Node):
    ...
    def structure_data(self, eex, configuration, triclinic=False):
        ...
        functional_forms = eex['functional_forms']
        ...
        if key not in configuration.atoms:
            ...
            for idx, xyz_atomtype, atom_q in zip(
               atom_idx , eex['atoms'], charges 
            ):
                x, y, z, atomtype = xyz_atomtype
                ...
                lines.append(
                    f'{idx:6d} {molecule_idx:6d} {atomtype:6d} {atom_q:6.3f} {x:12.7f} {y:12.7f} '
                    f'{z:12.7f}'
                )
        ...
        if 'n_bonds' in eex and eex['n_bonds'] > 0:
            ...
            use_hybrid = len(functional_forms['bond']) > 1
        ...
        if 'n_angles' in eex and eex['n_angles'] > 0:
            ...
            use_hybrid = len(functional_forms['angle']) > 1
            quartic_function = (
        ...
        if 'n_torsions' in eex and eex['n_torsions'] > 0:
            ...
            use_hybrid = len(functional_forms['torsion']) > 1
