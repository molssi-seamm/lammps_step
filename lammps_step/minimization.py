    ...
    def get_input(self, extras=None):
        ...
        if self.convergence == "normal":
            ...
            if self.etol_method == "is":
                ...
                raise RuntimeError("Variable handling not implemented for etol")
        elif "forces" in self.convergence:
            # force tolerance
            if self.ftol_method == "is":
                ftol = self.ftol.to("kcal/mol/Å").magnitude
            else:
                raise RuntimeError("Variable handling not implemented for ftol")
        else:
            raise RuntimeError("Variable handling not implemented for convergence")
        ...
        if maxiters is None:
            if self.maxiters_method == "default":
                ...
                raise RuntimeError("Variable handling not implemented for maxiters")
        ...
        if maxevals is None:
            if self.maxevals_method == "default":
                ...
                raise RuntimeError("Variable handling not implemented for maxevals")

        thermo_properties = (
            "press etotal ke pe ebond " "eangle edihed eimp evdwl etail ecoul elong"
        )
